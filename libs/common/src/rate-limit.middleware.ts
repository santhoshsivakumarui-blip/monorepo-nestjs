...
client.on("error", () => {
  // Redis may be unavailable during local startup or transient outages.
  // The middleware handles this gracefully at request time.
});
const policies = [
...
export async function rateLimit(
  ...
  const policy = policies.find((candidate) =>
    request.path.startsWith(candidate.prefix),
  );
  ...
  try {
    ...
    response.setHeader(
      "RateLimit-Reset",
      Math.ceil(((bucket + 1) * policy.windowMs) / 1000),
    );
    ...
    if (request.path.startsWith("/api/auth"))
      return response.status(503).type("application/problem+json").send({
